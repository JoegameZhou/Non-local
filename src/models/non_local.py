...
class NLBlockND(nn.Cell):
    def __init__(self, in_channels, inter_channels=None, mode='embedded',
        ...
        self.g = conv_nd(in_channels=self.in_channels, out_channels=self.inter_channels, kernel_size=1, has_bias=True, weight_init=HeNormal(mode='fan_out'))

        # add BatchNorm layer after the last conv layer
        if bn_layer:
            self.out = nn.SequentialCell(
                    conv_nd(in_channels=self.inter_channels, out_channels=self.in_channels, kernel_size=1, has_bias=True, weight_init=HeNormal(mode='fan_out')),
                    bn(self.in_channels, gamma_init="zeros", beta_init="zeros")
                    # from section 4.1 of the paper, initializing params of BN ensures that the initial state of non-local block is identity mapping
                )
        else:
            self.out = conv_nd(in_channels=self.inter_channels, out_channels=self.in_channels, kernel_size=1, has_bias=True, weight_init="zeros", bias_init="zeros")
            # from section 3.3 of the paper by initializing Wz to 0, this block can be inserted to any existing architecture

        # define theta and phi for all operations except gaussian
        if self.mode == "embedded" or self.mode == "dot" or self.mode == "concatenate":
            self.theta = conv_nd(in_channels=self.in_channels, out_channels=self.inter_channels, kernel_size=1, has_bias=True, weight_init=HeNormal(mode='fan_out'))
            self.phi = conv_nd(in_channels=self.in_channels, out_channels=self.inter_channels, kernel_size=1, has_bias=True, weight_init=HeNormal(mode='fan_out'))

        if self.mode == "concatenate":
            self.W_f = nn.SequentialCell(
                    nn.Conv2d(in_channels=self.inter_channels * 2, out_channels=1, kernel_size=1, weight_init=HeNormal(mode='fan_out')),
                    nn.ReLU()
                )
    ...
    def construct(self, x):
        ...
        # print("non-local attention:", f.shape)
        ...
        # ravel() here just allocates contiguous chunk of memory
...
class Bottleneck(nn.Cell):
    ...
    def __init__(self, inplanes, planes, time_kernel=1, space_stride=1, downsample=None, non_local=False):
        ...
        self.conv1 = nn.Conv3d(inplanes, planes, kernel_size=(time_kernel, 1, 1), padding=(int((time_kernel-1)/2), int((time_kernel-1)/2), 0, 0, 0, 0), pad_mode="pad",
                               has_bias=False, weight_init=HeNormal(mode='fan_out'))
        self.bn1 = nn.BatchNorm3d(planes)
        self.conv2 = nn.Conv3d(planes, planes, kernel_size=(1, 3, 3), stride=(1, space_stride, space_stride), pad_mode="pad", padding=(0, 0, 1, 1, 1, 1),
                               has_bias=False, weight_init=HeNormal(mode='fan_out'))
...
class I3DResNet(nn.Cell):
    ...
    """
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], frame_num=32, num_classes=400):
        self.inplanes = 64
        super(I3DResNet, self).__init__()
        self.conv1 = nn.Conv3d(3, 64, kernel_size=(5, 7, 7), stride=2, padding=(2, 2, 3, 3, 3, 3), pad_mode="pad", has_bias=False, weight_init=HeNormal(mode='fan_out'))
        self.bn1 = nn.BatchNorm3d(64)
        self.relu = nn.ReLU()
        ...
        self.maxpool2 = mindspore.ops.MaxPool3D(kernel_size=(3, 1, 1), strides=(2, 1, 1), pad_mode="pad", pad_list=(1,1,0,0,0,0))
        
        self.layer2 = self._make_layer_inflat(block, 128, layers[1], time_kernel=[3, 1, 3, 1], space_stride=2, non_local=True)

        self.layer3 = self._make_layer_inflat(block, 256, layers[2], time_kernel=[3, 1, 3, 1, 3, 1], space_stride=2, non_local=True)
    ...
    def _make_layer_inflat(self, block, planes, blocks, time_kernel, space_stride=1, non_local=False):
        ...
        if space_stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.SequentialCell(
                nn.Conv3d(self.inplanes, planes * block.expansion,
                          kernel_size=(1, 1, 1), stride=(1, space_stride, space_stride), has_bias=False, weight_init=HeNormal(mode='fan_out')),
                nn.BatchNorm3d(planes * block.expansion),
            )
    ...
    def construct_single(self, x):
        ...
        x = self.avgpool(x)
        x = self.avgdrop(x)
        x = x.view(batch_size, -1) #todo:x = x.view(x.shape[0], -1)为什么报错，此处的x.shape[0]为什么不是32即batch_size?
        x = self.fc(x)
...
def C2DResNet50(**kwargs):
    ...
    pretrained_dict_2d = load_checkpoint('/home/Non-Local-mindspore/src/utils/resnet50.ckpt')
...
if __name__ == '__main__':
    ...
    import numpy as np
    net = I3DResNet50()
    #print(load_checkpoint('/home/Non-Local-mindspore/src/utils/resnet50-nl.ckpt').keys())
    
    #for i,j in net.parameters_dict().items():
    #    print(i,end='\t\t\t\t\t\t\t\t')
    #    print(j.shape)
    #pretrained_dict_2d = load_checkpoint('/home/Non-Local-mindspore/src/utils/resnet50.ckpt')
    #l1=list(pretrained_dict_2d.keys())
        #print(j.data.asnumpy())
    #l2=list(net.parameters_dict().keys())
    #print(l1)
    #print(l2)
    #print(net.parameters_dict().keys())
    #print(net.parameters_dict()['conv1.weight'].data.asnumpy())
    #print(net.parameters_dict()['layer1.0.conv2.weight'].shape)
    #print(net.parameters_dict()['layer1.0.conv1.weight'].shape)
    #img = Tensor(np.arange(1  * 3 * 32 * 224 * 224).reshape((1, 3, 32, 224, 224)), mindspore.float32)
    #out = net(img)
    #print(out.shape)
    #for bn_layer in [True, False]:
    #    img = mindspore.ops.Zeros(2, 3, 20)
    #    net = NLBlockND(in_channels=3, mode='concatenate', dimension=1, bn_layer=bn_layer)
    #    out = net(img)
    #    print(out.shape)

    #    img = mindspore.ops.Zeros(2, 3, 20, 20)
    #    net = NLBlockND(in_channels=3, mode='concatenate', dimension=2, bn_layer=bn_layer)
    #    out = net(img)
    #    print(out.shape)

    #    img = mindspore.ops.StandardNormal((2, 3, 8, 20, 20))
